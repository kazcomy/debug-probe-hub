...
def execute_command(container_name, command, mode):
...
def main():
    ...
    if mode not in ["debug", "flash"]:
        print(f"Error: Invalid mode '{mode}'. Must be 'debug' or 'flash'", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        # Cleanup existing processes in debug mode
        if mode == "debug":
            cleanup_existing_processes(container_name, probe_id, interface)

        # Execute command
        result = execute_command(container_name, command, mode)
